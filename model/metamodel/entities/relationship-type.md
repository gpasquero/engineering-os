...
established-by: [ADR-0042, ADR-0065, ADR-0066]
...
A RelationshipType declares five things:

| Declares | States |
|---|---|
| **domain** | what may be the source |
| **range** | what may be the target |
| **cardinality** | how many, from each end |
| **constraints** | conditions the association must satisfy |
...
```text
RelationshipType   domain · range · cardinality · constraints · semantics
       ↓
   Compiler
       ↓
...
A declaration in the semantic model naming the five fields above.
...
**The set of framework relationship types is undeclared.** Every entity
specification in `entities/` has a relationships table using names — `scoped-to`,
`contains`, `realised-by` — that no RelationshipType declares. Those tables are
currently the only record that these types exist.

This is the same gap `BoundedContext` and `Actor` record: **the framework does
not yet model itself.** It closes in B3.
